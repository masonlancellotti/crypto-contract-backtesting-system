...
bound is provably pessimistic and cannot be tightened without trade prints or
sub-second WebSocket book data, which requires a (free) Kalshi API key. That, or
a structural change (cross-venue, different series), is where remaining edge
could live. The system itself is healthy: it measures honestly and fails fast.
...
| 10 | Stale-quote v2 on hi-res joins | `kalshi-reprice-lag-study --hires` | NEGATIVE: win 0%, no fee-surviving edge; +250/500ms response horizons ~0% covered because the Kalshi book is REST-polled at ~1.1s | BLOCKED on Kalshi WS (needs API key) |
...
| 16 | Kalshi market-data WebSocket feasibility | `kalshi-ws-feasibility` | Auth-gated even for market data (RSA-PSS); clean blocker without creds; `KalshiWSBookSource` ready behind `KALSHI_HIRES_BOOK_SOURCE=websocket` | BLOCKED on user: generate API key |
...
1. **Kalshi API key (free, user action)** → read-only WS book deltas + trade
   prints → resolves legs 10, 13, 14 (the only legs with genuine remaining
   upside at this venue) and enables honest queue/fill modeling.
2. **Public trade prints via REST** (`/markets/{ticker}/trades`) — weaker than
   WS but no auth; would partially tighten legs 13/14. Not yet collected.
